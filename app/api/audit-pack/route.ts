...
export async function POST(request: Request) {
  try {
    ...
    const d1 = getD1();
    const recoveryCase = await d1.prepare("SELECT * FROM recovery_cases WHERE id = ? AND workspace_id = ? LIMIT 1").bind(caseId, context.workspace.id).first<Record<string, unknown>>();
    ...
    const [sources, documents, fields, matches, applicability, receipts, receiptAllocations, authorityAllocations, outcomes, correspondence, notes, checklist, audit] = await Promise.all([
      ...
      d1.prepare("SELECT d.id, d.file_name, d.mime_type, d.size_bytes, d.sha256, d.kind, d.status, d.created_at FROM evidence_documents d JOIN case_sources s ON s.document_id = d.id WHERE s.case_id = ? AND s.workspace_id = ? UNION SELECT d.id, d.file_name, d.mime_type, d.size_bytes, d.sha256, d.kind, d.status, d.created_at FROM evidence_documents d WHERE d.id = ? AND d.workspace_id = ?").bind(caseId, context.workspace.id, recoveryCase.source_document_id ?? "", context.workspace.id).all(),
      d1.prepare("SELECT f.* FROM extracted_fields f JOIN evidence_documents d ON d.id = f.document_id WHERE d.workspace_id = ? AND d.id = ? ORDER BY f.field_name").bind(context.workspace.id, recoveryCase.source_document_id ?? "").all(),
      ...
      d1.prepare("SELECT r.* FROM receipt_records r JOIN receipt_allocations a ON a.receipt_id = r.id WHERE a.case_id = ? AND a.workspace_id = ? ORDER BY r.created_at").bind(caseId, context.workspace.id).all(),
      ...
      d1.prepare("SELECT a.*, r.record_reference, r.authority_type, r.credit_status FROM authority_allocations a JOIN authority_records r ON r.id = a.authority_record_id WHERE a.case_id = ? AND a.workspace_id = ? ORDER BY a.created_at").bind(caseId, context.workspace.id).all(),
