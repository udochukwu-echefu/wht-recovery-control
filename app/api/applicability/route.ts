...
import { isTerminalRecoveryCaseStage, nextActionForRecoveryCase, projectRecoveryCaseStage, stageForApplicabilityOutcome } from "@/lib/case-stage-policy";
...
export async function POST(request: Request) {
  try {
    ...
    const currentStage = projectRecoveryCaseStage(recoveryCase.stage);
    if (currentStage === "recognised" || isTerminalRecoveryCaseStage(currentStage)) return Response.json({ error: "Applicability cannot be changed after recognition or a terminal outcome." }, { status: 409 });
    const rules = await getActiveRuleSet(context, recoveryCase.businessDate ?? undefined);
    if (!rules.applicabilityCategories.includes(category)) return Response.json({ error: "Transaction category is not included in the active approved rule set." }, { status: 400 });
    const expectedRateBps = body.expectedRateBps === null || body.expectedRateBps === undefined ? null : Number(body.expectedRateBps);
    const suppliedExpectedKobo = body.confirmedExpectedKobo === null || body.confirmedExpectedKobo === undefined ? recoveryCase.expectedWhtKobo : Number(body.confirmedExpectedKobo);
    const confirmedExpectedKobo = ["not_applicable", "exempt"].includes(outcome) ? 0 : suppliedExpectedKobo;
    ...
    if (!Number.isInteger(confirmedExpectedKobo) || confirmedExpectedKobo < 0 || confirmedExpectedKobo > recoveryCase.invoiceGrossKobo) return Response.json({ error: "Confirmed WHT must be a valid non-negative kobo amount no greater than the invoice gross." }, { status: 400 });
    if (outcome === "applicable" && confirmedExpectedKobo === 0) return Response.json({ error: "An applicable WHT determination must record a positive expected amount." }, { status: 400 });
    if (["not_applicable", "exempt"].includes(outcome) && body.confirmedExpectedKobo !== undefined && Number(body.confirmedExpectedKobo) !== 0) return Response.json({ error: "Exempt or non-applicable determinations must record zero expected WHT." }, { status: 400 });
    ...
    const stage = stageForApplicabilityOutcome(outcome as "applicable" | "not_applicable" | "exempt" | "uncertain");
    ...
    const nextAction = outcome === "uncertain" ? "Escalate applicability review" : nextActionForRecoveryCase({ stage, exceptionCode, hasReceipt: false });
    const d1 = getD1();
    await d1.batch([
      ...
      d1.prepare("UPDATE recovery_cases SET applicability_status = ?, expected_wht_kobo = ?, stage = ?, exception_code = ?, rule_version = ?, next_action = ?, updated_at = ? WHERE id = ? AND workspace_id = ? AND client_id = ?")
        .bind(applicabilityStatus, confirmedExpectedKobo, stage, exceptionCode, rules.version, nextAction, now, caseId, context.workspace.id, context.clientId),
    ]);
